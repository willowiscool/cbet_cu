...
namespace consts {
	...
	constexpr size_t GRID = NX * NY * NZ;
	...
	constexpr size_t NRAYS = NRAYS_X*NRAYS_Y;
	...
	constexpr double DY = (YMAX-YMIN)/((double)NY-1.0);
	...
	constexpr double LASER_WAVE_LENGTH = 0.3510; // hard coded in main in cpp - willow
	constexpr double LAMBDA = 1.053e-4/3.0; // wavelength of light, in cm. This is frequency-tripled "3w" or "blue" (UV) light
	constexpr double W0 = 2.0*M_PI*C_SPEED/LAMBDA; // used in reading mesh and cbet, not sure what it is - willow
	constexpr double ZEFF = 3.9623225; // also doesn't come with a comment - willow
	...
	// TWO BEAM TESTCASE (TEMP)
#ifdef __CUDACC__
	__device__ constexpr double X01[] = {0.0, 0.0};
	__device__ constexpr double Y01[] = {YMIN, XMIN};
	__device__ constexpr double Z01[] = {0.0, 0.0};
	__device__ constexpr double KX01[] = {0.0, 1.0};
	__device__ constexpr double KY01[] = {1.0, 0.0};
	__device__ constexpr double KZ01[] = {0.0, 0.0};
#endif
}
